...
## G) Let's also try to make a T-sne plot.
tsne_input = d_num
tsne_out <- Rtsne(tsne_input, pca = F, check_duplicates = FALSE)

# H) Have a look at Rtsne in the help menu using ?Rtsne. Any of the parameters we want to change? Why?

# I) Have a look at the tsne_out which is the output of Rtsne (HINT: click on it in the environment).
...
# J) Make a plot of the output 
ggplot(tsne_plot) +
  geom_point(aes(x=x, y=y, color = ?))

# K) Try to use different parameters in Rtsne to improve the t-sne separation. You can also
